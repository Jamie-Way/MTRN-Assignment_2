...
namespace GamePad {
  ...
  class XBoxController {
    public:
      XBoxController(XInputWrapper* xinput, DWORD id);	//constructor takes in ID of controller and what? (commands made through xinputwrapper?)
      ~XBoxController() = default;

      // methods which report information relating to the specific controller
      DWORD GetControllerId();	//Collects the ControllerID, but why?
	  ...
	  //My function members
	  int GetRadius();			//Get the deadzone value
    ...
    private:
	  ...
	  //My data members
	  int Radius;				//the deadzone radius
	  int ControllerID;			//ID value of controller created
  };
};
